    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {data.map((item) => (
        <article key={item.id} className="group">
          <a href={item.link} target="_blank" rel="noopener" className="block h-full">
            <GlowCard className="h-full border-line group-hover:border-primary/50 transition-all">
              <div className="flex justify-between items-start mb-4">
                ...
                <time dateTime={item.date} className="flex items-center gap-1 text-[10px] text-text-muted font-bold uppercase">
