      ...
      <Container>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-stretch relative">
          ...
          {plans.map((plan) => (
            <div 
              key={plan.name} 
              className={`flex flex-col p-8 rounded-3xl relative overflow-hidden transition-all duration-300 ${
                plan.isFeatured 
                  ? "bg-[rgba(14,15,18,0.85)] border-2 border-[#E2C896] shadow-[0_0_40px_rgba(226,200,150,0.15)] scale-[1.03] z-10" 
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3 mb-6 relative z-10">
                ...
                <div className={`px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-lg shrink-0 flex items-center gap-1 ${
                  plan.isFeatured 
              ...
              <div className="mb-8 relative z-10">
                <div className="flex items-baseline gap-2 font-mono">
                  <span className="text-[#71717A] text-xl line-through opacity-70">${plan.regular}</span>
                  <span className="text-5xl font-black tracking-tighter text-[#FFFFFF]">${plan.price}</span>
                </div>
                ...
                <p className="text-xs text-[#E2C896] mt-2 uppercase tracking-widest font-bold font-mono">{plan.monthly}</p>
              </div>

              <p className="text-sm text-[#A1A1AA] leading-relaxed mb-8 border-t border-[rgba(243,208,129,0.08)] pt-6 relative z-10">
                {plan.desc}
              </p>
              ...
              <div className="space-y-3.5 mb-8 flex-grow relative z-10">
                {plan.features.map((f, i) => (
                  <div key={i} className="flex gap-3 items-start">
                    <Check className="w-4 h-4 text-[#E2C896] flex-shrink-0 mt-0.5" />
                    <span className="text-xs font-bold tracking-tight leading-tight text-[#FFFFFF]">{f}</span>
                  </div>
                ))}
              </div>
