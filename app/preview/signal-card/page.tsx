...
interface OverlayCoords {
  ...
  tp2Y: number | null;
  tp3Y: number | null;
  /** X pixel where the right-margin future space STARTS (right after last candle) */
...
// ─── Component ────────────────────────────────────────────────────────────────
export default function SignalCardPreviewPage() {
  ...
  const [entry, setEntry] = useState("2393.09");
  const [stopLoss, setStopLoss] = useState("2369.32");
  const [tp1, setTp1] = useState("2440.92");
  const [tp2, setTp2] = useState("2460.75");
  const [tp3, setTp3] = useState("2490.52");
  ...
  // Keep latest parsed numbers in a ref so the overlay callback is never stale
  ...
  useEffect(() => {
    ...
    // Toggle autoScale to force lightweight-charts to recalculate the autoscale provider bounds
    if (chartRef.current) {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Overlay coordinate recalculation — called after any pan/zoom/resize/tick
  // ─────────────────────────────────────────────────────────────────────────
  const recalcOverlay = useCallback(() => {
    ...
    // The future space starts where the LAST candle's RIGHT EDGE is.
    ...
    if (latestTimeRef.current !== null) {
      ...
      if (barCenterX !== null) {
        // barSpacing is 15px; add half to get the right edge of the bar
  ...
  useEffect(() => {
    ...
    const chart = createChart(el, {
      ...
      rightPriceScale: { 
        ...
        autoScale: true,
        scaleMargins: {
          top: 0.1,    // leave 10% space at the top
          bottom: 0.25 // leave 25% space at the bottom so candles don't overlap volume
        }
      },
      timeScale: {
        borderColor: "#1E242C",
        ...
        rightOffset: 16,   // Extra right offset (in bars) for the forecasting box
        barSpacing: 15,    // Larger bar spacing makes candles thicker
      },
    ...
    fetch(`/api/klines?symbol=${pair}&interval=${timeframe}&limit=60`)
      ...
      .then((data: any[]) => {
        ...
        // Removed global horizontal lines - they conflict with the position tool projection.
  ...
  // Recalc overlay immediately when prices change
  useEffect(() => { setTimeout(recalcOverlay, 80); }, [entryNum, stopNum, tp1Num, tp2Num, tp3Num, recalcOverlay]);

  // ─────────────────────────────────────────────────────────────────────────
  // Overlay geometry logic (clamped safely to chart bounds)
  // ─────────────────────────────────────────────────────────────────────────
  ...
  const eY  = entryY;
  const sY  = stopY;
  // Use the extreme TP for the bounding box
  ...
  return (
    <div className="min-h-screen bg-[#07080A] text-slate-100 font-sans pb-20">
      ...
      {/* ── TOP CONTROL PANEL (Not part of screenshot) ── */}
      {!hideButtons && (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg font-black tracking-wide text-white uppercase leading-none">
                    WYSIWYG Signal Studio
                  </h1>
                  <p className="text-[11px] text-slate-400 mt-0.5 flex items-center gap-1.5">
                    <Activity className="w-3 h-3 text-[#E39E2E]" />
                    What you see here is exactly what the bot generates
                  </p>
                </div>
              </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        // 2. Capture Image as Blob
                        await new Promise(r => setTimeout(r, 100)); // stabilize render
                        const blob = await htmlToImage.toBlob(captureRef.current, { quality: 1, pixelRatio: 2 });
                        if (!blob) throw new Error("Failed to generate image blob");

                        // 3. Generate Signal Text
                        ...
                        // 4. Send to Next.js API to push to Telegram Admin
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        // Small delay to ensure lightweight-charts rendering is stable
                      ...
                      {tgtH > 0 && (
                        <div
                          ...
                          style={{
                            ...
                            borderLeft: "2px solid #F6E09E", // Strong left boundary
                          }}
                        >
                          {/* TP Labels (pinned to the top/bottom based on value) */}
                          {(() => {
                            const validTargets = [
                            ...
                            .filter(Boolean)
                            .sort((a, b) => a!.relativeY - b!.relativeY); // Sort from top to bottom

                            // Vertical Anti-Collision (Relaxation algorithm)
                      ...
                      {slH > 0 && (
                        <div
                          ...
                          style={{
                            ...
                            borderLeft: "2px solid #ef5350", // Strong left boundary
                          }}
                        >
                          {/* SL Label (pinned to bottom) */}
