export const P0_URLS = [
  'https://www.yagacalls.com',
  'https://www.yagacalls.com/pricing',
  'https://www.yagacalls.com/method',
  'https://www.yagacalls.com/proof',
  'https://www.yagacalls.com/best-crypto-signals-group',
  'https://www.yagacalls.com/premium-crypto-signals-telegram',
  'https://www.yagacalls.com/crypto-signals-with-proof',
  'https://www.yagacalls.com/crypto-signals-with-risk-management',
  'https://www.yagacalls.com/free-vs-paid-crypto-signals',
  'https://www.yagacalls.com/regions',
  'https://www.yagacalls.com/regions/uae',
  'https://www.yagacalls.com/regions/dubai',
  'https://www.yagacalls.com/regions/uk',
  'https://www.yagacalls.com/regions/usa',
  'https://www.yagacalls.com/regions/singapore',
];

export const P1_URLS = [
  'https://www.yagacalls.com/regions/middle-east',
  'https://www.yagacalls.com/regions/saudi-arabia',
  'https://www.yagacalls.com/regions/qatar',
  'https://www.yagacalls.com/regions/europe',
  'https://www.yagacalls.com/regions/germany',
  'https://www.yagacalls.com/regions/switzerland',
  'https://www.yagacalls.com/regions/russia',
  'https://www.yagacalls.com/regions/gcc',
  'https://www.yagacalls.com/regions/australia',
  'https://www.yagacalls.com/blog',
  'https://www.yagacalls.com/academy',
];

export const P2_URLS = [
  'https://www.yagacalls.com/privacy',
  'https://www.yagacalls.com/risk-disclosure',
  'https://www.yagacalls.com/contact',
];

export const ALL_INDEXABLE_URLS = [...P0_URLS, ...P1_URLS, ...P2_URLS];
